  ...
  try {
    const [
      ...
      supabase
        .from("messages")
        .select(
          `id, content, created_at, transaction_id, sender:users!sender_id(id, name), receiver:users!receiver_id(id, name), transaction:transactions (id, requester:users!requester_id(id, name), owner:users!owner_id(id, name), resource:resources (id, title))`,
        )
